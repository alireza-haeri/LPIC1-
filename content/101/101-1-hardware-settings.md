...
سیستم عامل **لایه‌ای بین سخت‌افزار و برنامه‌ها** است. وقتی برنامه‌های کاربردی به منابعی نیاز دارند، سیستم عامل آن‌ها را مدیریت و تخصیص می‌دهد.
...
!!! note "تعریف"
    Firmware نوعی نرم‌افزار است که به‌طور دائم در سخت‌افزار ذخیره و اجرا می‌شود.
...
**UEFI** (رابط میان‌افزار توسعه‌یافته یکپارچه) استاندارد فعلی است:
...
!!! tip "مقایسه"
    UEFI نسبت به BIOS پیشرفته‌تر، سریع‌تر و امن‌تر است.
...
**PCI** (اتصال اجزای جانبی) به کاربران اجازه می‌دهد قطعات اضافی به مادربورد متصل کنند. امروزه اکثر سیستم‌ها از **PCI Express** یا **PCIe** استفاده می‌کنند.
...
**تفاوت اصلی:**
- **SSD:** بدون قطعات متحرک و با حافظه فلش کار می‌کند
- **HDD:** دارای بازوهای مکانیکی و صفحات چرخان است

**عمر مفید:**
- HDD: 3 تا 5 سال
- SSD: بیش از 10 سال (به دلیل مکانیزم Bad-Block-Management)

!!! info "مکانیزم Bad-Block-Management"
    SSD های مدرن دارای ظرفیت رزرو هستند که سلول‌های آسیب‌دیده را با سلول‌های سالم جایگزین می‌کنند.
...
**USB** (گذرگاه سریال جهانی) رابطی سریال با سرعت‌های مختلف:
...
**GPIO** (ورودی/خروجی عمومی) پین‌های ورودی/خروجی عمومی هستند که برای کنترل سخت‌افزارهای خارجی استفاده می‌شوند.
...
!!! note "تفاوت /sys و /dev"
    در `/sys` دستگاه‌ها بر اساس فناوری دسته‌بندی می‌شوند اما `/dev` انتزاعی است.
...
- قوانین سفارشی برای دستگاه‌ها اجرا می‌کند
...
!!! tip "قابلیت udev"
    می‌توانید با قوانین سفارشی نام‌گذاری دلخواه انجام دهید. مثلاً فلش مموری همیشه به عنوان `/dev/mybackup` شناخته شود و هنگام اتصال، اسکریپت پشتیبان‌گیری اجرا شود.
...
!!! warning "هشدار"
    تغییرات در `/proc` موقتی هستند و پس از ریبوت حذف می‌شوند. برای دائمی کردن باید فایل‌های پیکربندی در `/etc` را ویرایش کنید.

**تمرین - بررسی منابع سخت‌افزاری:**
...
**خروجی شامل:** نام، نویسنده، توضیحات، وابستگی‌ها، پارامترها
...
!!! warning "هشدار"
    استفاده از گزینه `-f` برای حذف اجباری می‌تواند باعث ناپایداری سیستم شود.

**تفاوت modprobe و insmod:**

| ویژگی | modprobe | insmod |
|--------|----------|--------|
| وابستگی‌ها | خودکار حل می‌کند | حل نمی‌کند |
| مسیر فایل | نیازی نیست | نیاز به مسیر کامل |
| توصیه | ✅ پیشنهاد می‌شود | ❌ توصیه نمی‌شود |
...
!!! tip "نکته"
    فایل‌های در `/etc/modprobe.d/` برای تنظیمات پیشرفته‌تر مناسب‌اند.
...
!!! example "نکات کلیدی برای آزمون"
    ...
    - نقش udev در مدیریت `/dev`
    - استفاده از sysfs در `/sys`
